  ...
  methods: {
      ...
      return this.getPassphrase()
        ...
        .then((text) => {
          if (this.passphrase === undefined) {
            ...
            setTimeout(() => (this.passphrase = undefined), 1000 * 60 * 5); // 5 minutes
          }
    ...
    fetchKeys() {
      ...
      return this.request("/keys.gpg")
        .then((res) => {
          if (res.status == 404) {
            return this.$refs["pgp-ask-key"].ask().then(readFile);
          }
        ...
        .then((armored) => {
          ...
          keys = keys.filter(
            (key) => key.verifyPrimaryKey() === openpgp.enums.keyStatus.valid
          );

          return this.$refs["pgp-ask-pass"]
            .ask()
            .catch(() => {
              throw errCancelled;
            })
